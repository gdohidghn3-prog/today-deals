...
export default function ConveniencePage() {
  ...
  const [allDeals, setAllDeals] = useState<Deal[]>(() => getConvenienceDeals());
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/deals?filter=convenience")
      .then((res) => res.json())
      .then((data) => {
  ...
  const filteredDeals = useMemo(() => {
    let deals = allDeals.filter((d) => d.source === activeStore);
    if (dealType !== "all") {
      deals = deals.filter((d) => d.discount === dealType);
    }
    return deals;
  }, [allDeals, activeStore, dealType]);

  const storeCounts = useMemo(() => {
    const counts: Record<string, number> = {};
    for (const store of STORES) {
      counts[store.key] = allDeals.filter((d) => d.source === store.key).length;
    }
    return counts;
  }, [allDeals]);
    ...
    <div className="max-w-3xl mx-auto px-4 pb-20">
      <div className="pt-6 pb-4">
        ...
        <p className="text-sm text-[#64748B] mt-1">
          1+1, 2+1 행사 모아보기 · 총 {allDeals.length}개
        </p>
      </div>
          ...
          {Array.from({ length: 9 }).map((_, i) => (
            <div key={i} className="bg-white border border-[#E2E8F0] rounded-xl overflow-hidden animate-pulse">
              ...
              <div className="p-2 h-[52px]">
                <div className="h-3 bg-[#F1F5F9] rounded w-3/4 mb-1.5" />
